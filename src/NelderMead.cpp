...
NelderMead::NelderMead(int dim, double error, int max_iter) {
  if (dim < 1) {
  ...
  m_enableIter = (max_iter == 0 ? false : true); // TIREI O CONST
  m_iter = 0;
  m_maxIter = max_iter;
  m_step = 0;
...
double *NelderMead::run(double _cost) {
  ...
  do {
    ...
    if(m_enableIter){
      m_iter++;
      iter_test = m_iter < m_maxIter;
    }
  } while (volume() > m_error && iter_test);
